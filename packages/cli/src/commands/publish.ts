...
export default class Publish extends Command {
  ...
  async run(): Promise<void> {
    ...
    if (flags.registry) {
      await this.publishToRegistry(flags.registry, appDir, bundlePath, flags.author);
    } else {
  ...
  private async publishToRegistry(registryUrl: string, appDir: string, bundlePath: string, author?: string): Promise<void> {
    ...
    const body = (await res.json()) as { name?: string; version?: string; error?: string };
