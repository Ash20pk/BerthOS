...
export function createOtelStepTracer(options: OtelStepTracerOptions = {}): StepTracer {
  ...
  return {
    async emit(event: AgentStepEvent): Promise<void> {
      ...
      const spanName = event.kind === "llm-turn" ? `chat ${event.agentName}` : `execute_tool ${event.toolName ?? "unknown"}`;

      const attributes: Attributes = {
        "gen_ai.operation.name": event.kind === "llm-turn" ? "chat" : "execute_tool",
