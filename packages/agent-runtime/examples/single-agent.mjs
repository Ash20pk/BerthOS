...
// Anthropic, no multi-agent composition. Start here before manager-crew.mjs
// or networked-crew.mjs. Requires ANTHROPIC_API_KEY — skips (not fails) if
// absent, same convention as ../test/*-milestone.mjs.
import { fileURLToPath } from "node:url";
import { dirname, join } from "node:path";
import { createAgent, createAnthropicProvider } from "../dist/index.js";
...
async function main() {
  ...
  try {
    ...
    const result = await agent.run("Write a file called hello.txt containing the text 'hi from @berth/agent-runtime', then read it back to me.");
