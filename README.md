...
| `terminal:attach:*` | Recorded and reported only | Not kernel-enforced by itself. Declaring it is what makes `berth dev` publish the ttyd port — loopback-bound, and gated by HTTP basic auth with a per-boot credential. Opt out with `expose: { terminal: false }`. |
...
- **The governance gate is not a sandbox.** It fails open by default and covers `Computer`/`Agent` tool calls only — not MCP tools, `Agent.asTool()` delegation, `berth rpc`, or the HTTP RPC bridge. See [Governance and scoping](#governance-and-scoping).
