...
- **The ttyd, VNC, and CDP ports `berth dev` publishes are unauthenticated and bound to all host interfaces.** On a routable network that's an open root shell. Don't run `berth dev` on untrusted LANs until this is fixed.
