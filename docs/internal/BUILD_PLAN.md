...
|---|------|--------------------|
| 0.1 | **Supply chain before publish** (REMEDIATION 6.6). Pin every `uses:` in `.github/workflows/` to a full commit SHA (verified: `publish-npm.yml` currently uses tag refs like `actions/checkout@v7`, and it holds `id-token: write`); digest-pin base images in `base.Dockerfile`; fix `dependabot.yml`'s claim that actions are "pinned"; add SBOM generation to the publish workflow. | Done when no workflow references a mutable ref, `dependabot.yml`'s comment is true, and the publish workflow emits an SBOM artifact. Verification: a workflow-lint script in `scripts/` that greps for unpinned `uses:` and fails CI — the negative control is a deliberately unpinned ref failing it. |
| 0.2 | **Publish to npm + PyPI** (LAUNCH_PLAN 1.1). Agents prepare: version sweep, `files` fields exclude tests, per-package READMEs, provenance flags, final dry run green. **The human gate:** maintainer sets `NPM_TOKEN`, runs `dry_run=false`. | Done when `npm i -g @berth/cli && berth doctor` works on a machine that has never seen the repo. Verification: a fresh-machine (or empty-container) install script recorded in `docs/internal/` with its output. |
