...
**5.5** — **Closed.** Fleet credentials in `~/.berthrc` at default 0644 (`util/fleet.ts:9-30`); passed as `Env` on `createContainer` so permanently visible in `docker inspect` (`container.ts:237`), including `BERTH_HTTP_RPC_TOKEN` (`:201`); `berth snapshot create` copied the whole container environment to `~/.berth/snapshots/.../env.json` with no mode (`snapshot.ts:132`) despite its own comment at `:77-81` claiming secrets aren't captured; `~/.berth/os/<name>.json` held the RPC bearer token at 0644 (`os-state.ts:58-61`).

**The mode fixes were the easy half and not the interesting one.** `~/.berth/os/<name>.json` and a snapshot's `env.json` are now 0600 in 0700 directories, with the `chmod`-after-write that `mode:` alone doesn't give you — `writeFile`'s `mode` is masked by the umask on creation and **ignored entirely for a file that already exists**, which every one of these files is on the second run. `~/.berthrc` is the exception on purpose: it is the developer's own file, so Berth warns once when a credential-carrying one is group- or world-readable and names the `chmod`, rather than silently rewriting a mode it didn't set or refusing to read it and breaking every existing `--fleet` invocation.

**The real change is that a credential never becomes container configuration.** Docker's `Env` cannot be redacted after the fact: it is readable from `docker inspect` for the container's whole life, copied verbatim into every `commit`, present in every fresh `docker exec` process, and is literally what `berth snapshot create` reads back out to build `env.json`. So `startContainer()` now splits the environment it was given by name (`secrets.ts`'s `isSecretEnvName`) and routes the credential half through a 0600 host file, bind-mounted read-only at `/run/berth/secrets.env`, which `entrypoint.sh` sources with `set -a` before any daemon or app starts. Same process environment for the app; `docker inspect` and `docker commit` see the names' absence instead of their values. No secrets, no mount — a container carrying nothing sensitive is byte-for-byte what it was before.

The classifier is deliberately broad and deliberately dumb (substring on `SECRET`/`TOKEN`/`PASSWORD`/`CREDENTIAL`/`API_KEY`/`ACCESS_KEY`/…, plus a `_KEY`/`_PAT` suffix rule for `AZURE_OPENAI_KEY`), because the costs are asymmetric: a false positive is unobservable, a false negative is a credential in `docker inspect` forever. `_PAT` started as a substring and classified `BERTH_MESH_KEY_PATH` as a secret via the "_PAT" in "KEY_PATH" — caught by the test asserting Berth's own non-credential names stay in `Env`, which is the only reason that test exists in both directions.

Two things fail closed rather than degrading quietly. `entrypoint.sh` exits 1 if `BERTH_SECRETS_FILE` is set and unreadable, because a sandbox that boots and then 401s against a model provider is far harder to diagnose from inside the container than a refusal is from outside it. And the serializer rejects a name the shell cannot represent instead of dropping it, since a silently missing credential fails later, somewhere unrelated.

**Snapshots get the same treatment twice.** Because `env` is read out of `docker inspect`, the split above already keeps credentials out of a snapshot — `createSnapshot()` strips them again anyway for a caller that assembles `env` by hand, records the withheld *names* in `metadata.redactedEnvNames` (a "contains no secrets" guarantee is only useful if what it cost is visible), and `berth snapshot restore` prints them. `BERTH_SECRETS_FILE` is dropped silently and separately: it points at a mount that exists only for the boot that made it, and replaying it into a restore on another machine would hit the fail-closed path above and refuse to boot the restored sandbox. That one was found by running the milestone, not by reading the code.

**Verified against a real container, in both directions.** `secrets-milestone.mjs` asserts `docker inspect` carries neither the RPC token nor a provider key under any name, the host file is 0600 in a 0700 dir, `docker exec env` sees neither, a real `createSnapshot()`'s `env.json` and `image.tar` contain neither byte-string, and the host file is gone after `stopContainer()` — **and** that the HTTP RPC bridge authenticates the bearer token it was never given in `Env`. That last one is the point: every absence check here is trivially satisfied by never delivering the credential at all, so each is paired with a functional one. `published-port-security-milestone.mjs` covers the two non-Node consumers `entrypoint.sh` starts itself — its assertion that `BERTH_TERMINAL_CREDENTIAL` was *present* in `Env` is inverted, and ttyd and x11vnc still refuse anonymous and wrong credentials and accept the generated ones.

**Deliberately not claimed**, in full in [docs/secrets-reference.md](../secrets-reference.md#what-this-does-not-protect-against): Docker socket access is root-equivalent and can read the mounted file (what closed is the much weaker requirement of *merely being able to inspect metadata*, or of receiving someone else's snapshot); everything inside the container shares the environment, so there is no per-app secret scoping and a companion app sees the primary's keys; nothing is encrypted at rest (5.4); there is no rotation and no store behind the seam, so values still come from the caller's own environment; and `--fleet` env lands in the provider's control plane, where a Kubernetes Pod spec shows it to `kubectl get pod -o yaml` — Berth creates no `Secret` objects. `grants-server/src/operator-token.ts:19` was already right and is unchanged.
...
**The injected probe hid a real bug, exactly where it was predicted to.** The first run against a kernel with `landlock` in its LSM stack (WS1.3's Colima recipe) reported `UNKNOWN`, not `ACTIVE`: the probe resolved its scratch path with `tempfile.gettempdir()` *after* `restrict_self()` on a ruleset granting nothing, and `gettempdir()` looks for a writable directory by creating a file in each candidate — so on an enforcing kernel it raised and the probe printed a traceback instead of JSON. The single class of host where the answer was `ACTIVE` was the class that could not report it, and all 20 unit tests stayed green throughout, because they inject a fake probe rather than executing the Python. Fixed by resolving the path before restricting. Worth keeping as evidence for how much an injected-probe test is and isn't worth.

**Not closed by this.** Still open from the original entry: no Windows/WSL guidance, and build time is still never stated, so a first-timer cannot tell a slow cold build from a hang. `berth doctor --fix` does not exist — every failing check carries a `remedy` string, but nothing acts on it. ~~The `enforcing` verdict has not been observed on real hardware~~ — **observed 2026-08-18** on macOS via Colima (guest kernel 6.8.0-117-generic, Ubuntu 24.04, Landlock ABI 4): `berth doctor` → `enforcement: ACTIVE`, exit 0, all four checks green, recorded in [docs/mac-enforcement.md](../mac-enforcement.md) (WS1.3). A CI run on `ubuntu-latest` is still worth having as a regression gate, but it is no longer what the claim rests on.
