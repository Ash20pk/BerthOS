...
**WS0.2 changed this workstream's shape.** 5.1 and 5.3 were already built on unmerged branches and are now on `main`, so 2.1 is a verification task rather than a build task, and TLS — which this plan didn't list at all — is available to talk about at launch. That left **2.2 (secrets) as the only WS2 item still needing to be written**, which mattered because 2.2 is the one the launch gate requires. **2.2 is now done** — see `REMEDIATION.md` 5.5 and `docs/secrets-reference.md`. WS2's remaining item, 2.3, is the ops floor and is not gating.
...
| 2.2 | ~~**5.5 Secrets.**~~ **Done.** Credentials no longer enter Docker's `Env` at all — `startContainer()` routes every secret-named variable through a 0600 host file bind-mounted at `/run/berth/secrets.env` that `entrypoint.sh` sources before anything starts, which is also what keeps them out of `docker commit` and out of snapshots. 0600-in-0700 modes on `~/.berth/os/<name>.json` and snapshot `env.json`, a warning (not a silent chmod) on a credential-carrying group-readable `~/.berthrc`, snapshot env scrubbing that records the names it withheld so a restore can say what it's missing, and a "what this does not protect against" section in both `docs/secrets-reference.md` and the threat model — the honest limit being that Docker socket access is root-equivalent and reads the file regardless. | **Met, verified on a real container** (`secrets-milestone.mjs`, 20 checks): `docker inspect` shows no provider API key and no RPC token under any name, `docker exec env` sees neither, a real snapshot's `env.json` **and** `image.tar` contain neither, and the bridge still authenticates the token it was never given in `Env` — the paired functional check without which "absent" is satisfiable by never delivering it. `published-port-security-milestone.mjs` proves the same for ttyd and x11vnc, and its old assertion that the credential *was* in `Env` is inverted. |
