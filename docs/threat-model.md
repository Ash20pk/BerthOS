...
| B5 | Container → internet | The egress broker and the GitHub API broker | **Real for egress, partial for GitHub.** The egress broker scopes by port (80/443 unless the scope names one), refuses loopback/RFC1918/link-local/IMDS under every pattern including `*`, and dials the address it validated rather than re-resolving (*1.8*, closed). The GitHub scope heuristic still defaults to *allow* (*1.9*) |
...
- **Broker gaps (~~*1.8*~~ closed, *1.9* open).** The egress broker's holes are shut: ports are scoped, `*` no longer reaches IMDS/loopback/`host.docker.internal`, and DNS is pinned so the address dialled is the one validated. What remains is GitHub: scope classification defaults to `repos` rather than deny. An app declaring both `github:*` and `network:host:*` gets a raw CONNECT tunnel with no path inspection.
