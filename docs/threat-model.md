...
- **In-container privilege regain (*1.3*).** `unshare -Urm` restores the full capability set, because the container holds `CAP_SYS_ADMIN` for the FUSE mount and Docker's default seccomp profile relaxes its namespace restrictions when that cap is present. Landlock's filesystem rules still bind (inode-based, they survive namespace tricks); every other `SYS_ADMIN`-gated syscall comes back.
...
| [multi-app-reference.md](./multi-app-reference.md) — residual risk | B3/B4, *1.4*, `docker exec` bypass |
