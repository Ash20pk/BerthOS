...
- **Secrets are file-mode protected, not vault-managed (*5.5*, closed to its stated bar).** Credentials no longer reach `docker inspect`, an image layer, or a snapshot, and every file Berth writes that holds one is 0600 in a 0700 directory. What that does *not* cover, stated in full in the [secrets reference](./secrets-reference.md#what-this-does-not-protect-against): anyone with Docker socket access (root-equivalent — they can read the mounted file), anything inside the container (the app is meant to have the key, and there is no per-app secret scoping, so a companion app in an `--apps` sandbox sees the primary's), nothing encrypted at rest, no rotation, no secret-store backend behind the seam, and `--fleet` deploys whose env lands in the provider's own control plane (a Kubernetes Pod spec shows it in `kubectl get pod -o yaml`; Berth creates no `Secret` objects).
