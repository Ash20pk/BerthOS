...
| A2 | **Credentials** | LLM API keys and `GITHUB_TOKEN` in the container environment — since *5.5* delivered through a 0600 host file mounted at `/run/berth/secrets.env`, not through Docker's `Env`, so no longer in `docker inspect` or in any snapshot; fleet credentials in `~/.berthrc` (still the developer's own file to protect), the grants-server operator token, registry per-name owner tokens, and mesh peer tokens. `BERTH_TOKEN_SECRET` no longer exists (*1.10*). See [secrets reference](./secrets-reference.md) |
...
**T5 — An unprivileged local user on the host or CI runner.** Narrowed by *5.5*, not closed. Container credentials, `~/.berth/os/<name>.json` (the RPC bearer token), the per-boot secrets file, and snapshot `env.json` are now 0600 in 0700 directories, and snapshots carry no credential values at all — so this adversary no longer gets the keys by reading a world-readable file. What is left is `/context/*.json` at 0644 (conversation history and checkpoints, *5.4*), `~/.berthrc` if the developer left it group-readable (Berth warns, and deliberately does not chmod someone else's file), and — decisively — anyone in the `docker` group, who is root-equivalent on the host and can simply `docker exec` in.
...
|---|---|---|---|
| B1 | Model output → tool invocation | The governance gate (`evaluate_action`) and human approval | **Advisory, but no longer selective or avoidable.** Governance fails *closed* by default (*1.11*) and sits on the Computer's dispatch rather than one tool array, so every resident-app call through it is gated, as are MCP tools (`mcp:<server>`) and delegation (`agent:<name>`). A second gate in `@berth/sdk` covers the transports that never touch a Computer — the relay (`berth rpc`, `berth mcp`), the HTTP bridge, the TCP listener and a sibling's peer socket — announcing each with the caller the kernel established (*1.13*, closed). Human approval is fail-closed by design but is swallowed by the tool-error handler (*3.4*) |
...
| B3 | App process → sibling app process | Distinct uids; an app's own RPC socket is `0600`, and a caller declaring `app:invoke:<name>` gets its own socket in a directory only it can traverse — so the target both authorizes and identifies it | The grant is per-app, not per-export — a governor can now police the export, since every peer-socket call reaches its `evaluate_action` with the caller's name (*1.13*), but DAC itself still authorizes at connect; one shared PID namespace (*1.11*) |
...
| B5 | Container → internet | The egress broker and the GitHub API broker | **Real.** The egress broker scopes by port (80/443 unless the scope names one), refuses loopback/RFC1918/link-local/IMDS under every pattern including `*`, and dials the address it validated rather than re-resolving (*1.8*, closed). The GitHub broker maps paths to scopes from an explicit table that denies by default, normalizes `..` before checking and forwards what it checked, and owns `api.github.com` exclusively — the egress broker refuses that host for any app the dedicated broker is running for (*1.9*, closed) |
...
| **Host** | Credentials delivered by a 0600 bind-mounted file, never as Docker `Env`; 0600/0700 modes on every credential file Berth writes; snapshot env scrubbing | who can read a sandbox's API keys and bearer tokens *without* Docker socket access, and what a snapshot carries to another machine | `secrets-milestone.mjs`; `published-port-security-milestone.mjs` Test 3 |
...
- **`CAP_SYS_ADMIN` is no longer granted to the sandbox (*1.3* remainder closed — BUILD_PLAN M1.1), with a named fallback.** The semantic-FS FUSE mount moved to a per-sandbox sidecar container ([design](./internal/design/sys-admin-drop.md)); a booted sandbox's `docker inspect` shows no `SYS_ADMIN`, no `/dev/fuse`, and no AppArmor exception, and `mount(2)` inside it fails `EPERM` for every process, root daemons included — asserted with a negative control (`sys-admin-drop-milestone.mjs`: a `BERTH_DISABLE_FS_SIDECAR=1` boot has the cap back and the same mount succeeds). What remains, stated plainly: the *sidecar* still holds `SYS_ADMIN` (one daemon, no app code — narrowing it post-mount is M1.2's territory); a host without `rshared` bind propagation falls back to the pre-M1.1 in-sandbox mount, loudly and visibly in `inspect`; and the context-bus and mesh daemons still start before `agent-init` inside the sandbox with no seccomp filter and no Landlock domain (B4).
...
- ~~**Broker gaps (*1.8*, *1.9*).**~~ **Closed.** The egress broker's holes are shut: ports are scoped, `*` no longer reaches IMDS/loopback/`host.docker.internal`, and DNS is pinned so the address dialled is the one validated. The GitHub broker no longer classifies by path position — an explicit route table denies anything it doesn't cover, so `github:read:repos` stops carrying `/user/emails`, `/gists` and `/notifications` with it — and the two brokers now compose: an app declaring both `github:*` and `network:host:*` no longer gets a raw CONNECT tunnel to `api.github.com` alongside the inspected one. What is left here is scope rather than a hole: the route table covers GitHub's API partially, and an uncovered endpoint is refused.
...
- ~~**Governance gate coverage (*1.13*).**~~ **Closed, in two layers.** MCP tools and `Agent.asTool()` are gated under `mcp:<server>` and `agent:<name>`, and that gate moved onto the Computer's dispatch so it can't miss a tool that wasn't in one array. The transports that never touch a Computer — `berth rpc`, `berth mcp`, the HTTP bridge, the TCP listener, a sibling's direct socket call — are gated at `@berth/sdk`'s `invokeExport()`, where they all converge, with the caller taken from which listener accepted the connection rather than from the request. Two things stay outside it and always did: root on the host, who reaches the relay via `docker exec` (so for that channel the gate is policy and audit, not a boundary), and the kernel layer, which has no per-syscall hook to consult a verdict-provider. One cost: an agent's tool call passes both gates, so a governor sees two `evaluate_action` calls for it.
...
- **Enterprise-grade operational security is incomplete** (*Phase 5*): no identity/tenancy/RBAC (*5.2*), nothing encrypted at rest (*5.4*), no rate limiting or `/health` anywhere (*5.6*), no migration runner on any SQLite database (*5.7*), and a registry whose first publish of a name needs no auth against a 100 MB body limit. The audit trail (*5.1*) and TLS (*5.3*) are built, off by default, and documented for what they do and don't prove.
- **Secrets are file-mode protected, not vault-managed (*5.5* closed to its stated bar; per-app scoping now real — M1.3).** Credentials no longer reach `docker inspect`, an image layer, or a snapshot, and every file Berth writes that holds one is 0600 in a 0700 directory. A secret an app declares under `secrets:` in `berth.yml` is additionally scoped to that app: delivered via `/run/berth/secrets.<app>.env` (0600, the app's uid), sourced only in its own process tree, and unreachable from a sibling by env, `/proc/<pid>/environ`, or DAC — verified with a control boot proving the checks can fail (`per-app-secrets-milestone.mjs`). What that does *not* cover, stated in full in the [secrets reference](./secrets-reference.md#what-this-does-not-protect-against): anyone with Docker socket access (root-equivalent), *undeclared* secrets (which keep the shared-file behavior deliberately), the pre-`agent-init` root daemons (B4 — they can read any file in the container), nothing encrypted at rest, no rotation, no secret-store backend behind the seam, and `--fleet` deploys whose env lands in the provider's own control plane (a Kubernetes Pod spec shows it in `kubectl get pod -o yaml`; Berth creates no `Secret` objects).
...
- **Snapshots capture no credential at all.** `BERTH_TOKEN_SECRET` is gone with capability tokens (*1.10*), and since *5.5* `env.json` holds the non-secret environment only, at 0600, with the withheld names listed in `metadata.redactedEnvNames` so a restore can say what it is missing. The principle stands and is now enforced rather than asserted: restoring a stale secret is a regression, not a feature. The consequence is deliberate — a restored sandbox boots without the original's API keys and you supply them again. ([snapshots reference](./computer-snapshots-reference.md#whats-explicitly-deferred-named-here-not-silently-promised))
...
**Not reasonable yet.** Installing a resident app or a `berth.yml` you haven't read (T3 has no boundary at all). Running mutually-distrusting apps in one container and expecting the per-app rulesets to keep them apart (B3, B4). Publishing a sandbox's ports beyond loopback with `BERTH_PUBLISH_HOST` on an untrusted network — the credentials are real, but they're the only thing there (T4). Putting PHI, PII, or regulated data through it (*5.4* — nothing is encrypted at rest, and conversation history is persisted 0644). Treating the secrets work as a vault: it keeps credentials out of container metadata and off other local users' reads, and it is not a boundary against anyone holding the Docker socket ([secrets reference](./secrets-reference.md#what-this-does-not-protect-against)). Multi-tenant use of the grants server or registry (*5.2*).
...
| [secrets-reference.md](./secrets-reference.md) — what this does not protect against | A2, T5, *5.5* in Open gaps, the **Host** credential-delivery tier |
