...
|---|---|---|---|
| B1 | Model output → tool invocation | The governance gate (`evaluate_action`) and human approval | **Advisory.** Governance fails *open* by default and covers only `Computer`/`Agent` tool arrays — not MCP tools, `Agent.asTool()`, `berth rpc`, `berth mcp`, or the HTTP RPC bridge (*1.13*). Human approval is fail-closed by design but is swallowed by the tool-error handler (*3.4*) |
...
| B3 | App process → sibling app process | Distinct uids; each app's RPC socket `0660` in a `0710` directory it owns, reachable only by a caller declaring `app:invoke:<name>` | No `SO_PEERCRED` check, so a permitted caller is not *identified* and the grant is per-app, not per-export (*1.4* part 3); one shared PID namespace (*1.11*) |
...
- **Cross-app capability borrowing (*1.4*, partly closed).** An app's RPC socket now lives in `/run/berth/<app>/`, mode `0710` and owned by that app's uid, so a sibling reaches it only by declaring `app:invoke:<name>` — enforced by DAC, since Landlock does not gate `connect()` to a pathname Unix socket (see [per-app uid design](./per-app-uid-design.md)). What remains: the grant is connect-time, so an authorized caller reaches the target's *whole* export surface, and the serving app cannot tell which sibling is calling — that needs `SO_PEERCRED` (*1.4* part 3, *1.14*). The context-bus, semantic-FS, and mesh control sockets stay reachable by every app by design, and are still served by root daemons outside any Landlock domain (B4).
...
- **Governance gate coverage (*1.13*).** MCP tools, `Agent.asTool()`, `berth rpc`, `berth mcp`, the HTTP RPC bridge, and direct socket calls are all ungated.
...
- **Snapshots deliberately do not capture `BERTH_TOKEN_SECRET`** — restoring a stale secret would be a regression, not a feature. They *do* capture the container environment to an unmodded `env.json` (*5.5*), which is a gap, not a decision. ([snapshots reference](./computer-snapshots-reference.md#whats-explicitly-deferred-named-here-not-silently-promised))
...
| [governance-reference.md](./governance-reference.md) — fail-open, why not the kernel | B1, *1.13*, out-of-scope list |
