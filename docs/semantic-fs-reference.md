...
This is a real choice, not an oversight: `modernc.org/sqlite` is pure Go with no C extension support, so `sqlite-vec` and friends aren't available, and a brute-force scan is genuinely fine at the scale one sandbox's `/context` reaches — hundreds to low thousands of tagged files, where the embedding round trip in `@berth/sdk` dominates anyway. It is *not* fine as a general document store, and nothing here degrades gracefully if it's used as one: there's no pagination, no index on any scored column, and no early termination. A workload that needs more wants a real vector index behind the same `query()` interface, which is a different piece of work than tuning this one.
