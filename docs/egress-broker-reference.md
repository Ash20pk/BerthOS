...
What this can't do: **path/verb-level** API scoping (`github:read:repos` vs `github:write:issues`) needs to see inside the TLS session, which does need real interception. `examples/github-assistant` demonstrates what such an app's code looks like, but it isn't wired into the sandbox/Landlock pipeline as a deployed `apps/*` resident app (no milestone test exercises it against a live sandbox) — so this remains genuinely untested against anything real, and is left deliberately out of scope here rather than built against a hypothetical.
