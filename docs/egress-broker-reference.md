...
What this can't do: **path/verb-level** API scoping (`github:read:repos` vs `github:write:issues`) needs to see inside the TLS session, which does need real interception. `apps/github-assistant` is now a real deployed resident app (see `packages/docker-orchestrator/test/github-assistant-milestone.mjs`), so there's a real consumer to test against — but that test only proves the app's GitHub API calls work over a granted `network:connect:<port>`, host-level like everything else this broker does. The verb/path distinction itself (`github:read:*` vs `github:write:*`) is still genuinely unbuilt and would need a real MITM broker (CA generation, per-host leaf certs, decrypt/re-encrypt) — deliberately out of scope here, tracked as separate follow-up work.
