...
- **Local Docker only.** None of the `DeployAdapter` implementations (E2B, Daytona, K8s) expose anything like `invokeAppExport` yet, so there's no RPC bridge to a remote fleet instance today. `createAgent()`/`Computer.boot()` target local Docker, same as `berth dev`.
