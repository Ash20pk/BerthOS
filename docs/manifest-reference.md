...
`app:invoke:<name>` lets this app call another resident app's exports directly, over that app's RPC socket, inside the same container. It is the *only* way to do so: since [REMEDIATION 1.4](../REMEDIATION.md#14--app-rpc-sockets-in-world-writable-tmp-unauthenticated) each app's socket lives in `/run/berth/<app>/`, mode `0710` and owned by that app's own uid, and declaring this capability is what puts the caller in the target's group at boot. An app that declares nothing gets `EACCES` on `connect(2)` — from the kernel, not from a check in the SDK. Undeclared, it is also what `@berth/agents` generates for a synthesized agent app, one line per sibling whose exports it embeds as tools.

Two limits worth knowing. It is a connect-time gate, so it says *who may call*, not *which exports they may call* — the target's whole export surface is reachable once granted. And the serving app cannot yet tell which sibling is calling; that needs `SO_PEERCRED` (Step 4 of the [per-app uid design](./per-app-uid-design.md)). Naming an app that isn't in the same container is a warning at boot, not an error, since there is nothing to grant.
